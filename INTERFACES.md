...
| `/.well-known/core` | GET | — | `application/link-format` (ct=40), CoRE Link Format per RFC 6690 listing every resource above with `rt`/`if`/`ct` attributes | served from a string macro generated by [tools/gen_descriptors.py](tools/gen_descriptors.py) from [interface/coap.yaml](interface/coap.yaml), so the advertised resource set tracks the registered one automatically |
...
- ~~**No CoAP resource discovery.** There is no `/.well-known/core` resource, so a
  client cannot enumerate resources over CoAP (see proposal below).~~ **Resolved**
  by [TODO/02 §2.5](TODO/02-coap-fixes.md#25-no-coap-resource-discovery-wellknowncore--applied) — the resource is now served, with its
  payload generated from [interface/coap.yaml](interface/coap.yaml) via
  `tools/gen_descriptors.py`.
